...
const Expenses: React.FC = () => {
  ...
  const generateExpensePDF = async () => {
    try {
      ...
      yPosition += 8;
      pdf.setFont('helvetica', 'normal');
      pdf.text(`Total Budget: ₹${totalBudget.toLocaleString()}`, margin, yPosition);
      yPosition += 6;
      pdf.text(`Used Amount: ₹${usedAmount.toLocaleString()}`, margin, yPosition);
      yPosition += 6;
      pdf.text(`Remaining Amount: ₹${remainingAmount.toLocaleString()}`, margin, yPosition);
      yPosition += 6;
      pdf.text(`Due Amount: ₹${dueAmount.toLocaleString()}`, margin, yPosition);
      yPosition += 6;
      pdf.text(`Total Expenses: ${items.length}`, margin, yPosition);
      yPosition += 15;
      ...
      const colPositions = {
        ...
        title: margin + 20,
        category: margin + 80,
        amount: pageWidth - margin - 60,
        status: pageWidth - margin - 25,
      };

      // Header text
      pdf.text('S.No', colPositions.sno, yPosition + 2);
      pdf.text('Title', colPositions.title, yPosition + 2);
      pdf.text('Category', colPositions.category, yPosition + 2);
      pdf.text('Amount', colPositions.amount, yPosition + 2);
      pdf.text('Status', colPositions.status, yPosition + 2);
      ...
      items.forEach((expense, index) => {
        ...
        pdf.text(expense.title || 'N/A', colPositions.title, yPosition + 2);
        pdf.text(expense.category || 'N/A', colPositions.category, yPosition + 2);
        pdf.text(`₹${expense.amount?.toLocaleString() || '0'}`, colPositions.amount, yPosition + 2);
        pdf.text(expense.status || 'N/A', colPositions.status, yPosition + 2);
  ...
  const handleShareExpenses = async () => {
    try {
      const pdf = await generateExpensePDF();
      const blob = pdf.output('blob');
      const projectName = activeSite?.name || 'Project';
      const filename = `${projectName}_Expenses_${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await (navigator as any).share({
            ...
            text: `Expense report for ${activeSite?.name || 'Project'}`,
            files: [file],
          });
          return;
        } catch (err) {
      ...
      const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(
        `Expense Report for ${activeSite?.name || 'Project'}\n\nPlease find attached the expense report document.\n\nGenerated by IntirorHub`
      )}`;
      ...
      console.error('Failed to share expenses:', err);
      alert(`Failed to share expenses: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
